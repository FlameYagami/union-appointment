...
public class IpUtils {
    ...
    public static String getIpAddress(HttpServletRequest request) {
        ...
        // 多级ip段拆解, 只记录原始ip
        if (StrUtil.isNotEmpty(ip) && ip.contains(CommonConstant.COMMA)) {
            ip = ip.split(CommonConstant.COMMA)[0];
        }

        return "0:0:0:0:0:0:0:1".equals(ip) ? LOCAL_IP : new HTMLFilter().filter(ip);
    }
